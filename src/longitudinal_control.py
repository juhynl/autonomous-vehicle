...
class LongitudinalController:
    ...
    def PID_step(self, speed, target_speed):
        ...
        # derive PID elements
        proportional_element = self.KP * error
        integral_element = self.KI * max(self.sum_error, error)
        derivative_element = self.KD * (error - self.last_error)
        
        u = proportional_element + integral_element + derivative_element
        
        self.last_control = u
        self.last_error = error
        self.sum_error = max(self.sum_error, error)
        return u
